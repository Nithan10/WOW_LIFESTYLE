...
const VideoCard = ({ video, index, theme }: { video: typeof TRENDING_VIDEOS[0], index: number, theme: 'dark' | 'light' }) => {
  ...
  return (
    <motion.div
      ...
      onClick={togglePlay}
      className={`group relative ${getCardGradient()} rounded-2xl overflow-hidden cursor-pointer shadow-2xl border ${getBorderColor()} transition-all duration-500`}
    >
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-between z-20">
        <div className="flex justify-between items-start">
          ...
          <motion.div initial={{ x: 20, opacity: 0 }} animate={isHovered ? { x: 0, opacity: 1 } : {}} className="flex gap-2">
            <button className={`p-2.5 backdrop-blur-md rounded-full hover:bg-[#D4AF37] transition-colors ${theme === 'light' ? 'bg-white/80' : 'bg-black/50'}`} onClick={(e) => { e.stopPropagation(); toggleMute(); }}>{isMuted ? <VolumeX size={16} /> : <Volume2 size={16} />}</button>
          </motion.div>
        </div>
        <div className="space-y-3">
          <motion.h3 animate={isHovered ? { y: 0 } : { y: 10 }} className={`text-xl font-bold leading-tight ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>{video.title}</motion.h3>
        ...
        <motion.div animate={{ scale: isPlaying ? (isHovered ? 1.1 : 0) : 1, opacity: isPlaying ? (isHovered ? 1 : 0) : 1 }} className="absolute inset-0 flex items-center justify-center">
           <div className={`p-6 backdrop-blur-md rounded-2xl border ${theme === 'light' ? 'bg-white/80 border-gray-300' : 'bg-black/50 border-white/20'}`}>{isPlaying ? <Pause size={32} /> : <Play size={32} />}</div>
        </motion.div>
      </div>
    </motion.div>
  );
...
const CharacterSlider = ({ theme }: { theme: 'dark' | 'light' }) => {
  ...
  return (
    <div className={`w-full py-20 border-t ${theme === 'light' ? 'border-gray-200 bg-gray-50' : 'border-white/5 bg-[#080808]'}`}>
      <div className="max-w-7xl mx-auto px-4 mb-12 text-center">
         ...
         <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={`text-4xl md:text-5xl font-black uppercase tracking-tight ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>Shop By Character</motion.h2>
      </div>
      <div className="relative w-full overflow-hidden group">
        <div className={`absolute left-0 top-0 bottom-0 w-24 z-20 bg-gradient-to-r ${theme === 'light' ? 'from-gray-50' : 'from-[#080808]'} to-transparent pointer-events-none`} />
        <div className={`absolute right-0 top-0 bottom-0 w-24 z-20 bg-gradient-to-l ${theme === 'light' ? 'from-gray-50' : 'from-[#080808]'} to-transparent pointer-events-none`} />
        <motion.div ref={sliderRef} className="flex gap-6 px-4 md:px-12 cursor-grab active:cursor-grabbing pb-12 overflow-x-auto scrollbar-hide snap-x">
           {CHARACTERS.map((char, i) => (
             <motion.div key={char.id} initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05, duration: 0.5 }} className="relative flex-shrink-0 snap-center">
                <div className="group/card relative w-48 h-64 md:w-56 md:h-72 rounded-[2rem] overflow-hidden transition-all duration-500 hover:-translate-y-3">
                   ...
                   <div className={`relative w-full h-full bg-gray-900 rounded-[2rem] overflow-hidden border ${theme === 'light' ? 'border-gray-200' : 'border-white/10'} group-hover/card:border-white/30 transition-colors`}>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col items-center">
                         <h3 className="text-white font-bold text-lg uppercase tracking-wider text-center drop-shadow-md translate-y-2 group-hover/card:translate-y-0 transition-transform duration-300">{char.name}</h3>
                         <div className="h-1 w-8 rounded-full mt-3 transition-all duration-300 transform scale-x-0 group-hover/card:scale-x-100" style={{ backgroundColor: char.color }} />
                      </div>
                   </div>
                </div>
             </motion.div>
...
const BentoGrid = ({ theme }: { theme: 'dark' | 'light' }) => {
  return (
    <section className={`py-24 relative ${theme === 'light' ? 'bg-white' : 'bg-black'} border-t ${theme === 'light' ? 'border-gray-200' : 'border-white/10'}`}>
       <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16 flex flex-col md:flex-row justify-between items-end gap-6">
             <div className="max-w-2xl">
                ...
                <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className={`text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                   Best of <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#FCEEAC]">WOW Lifestyle</span>
                </motion.h2>
             </div>
             <motion.button initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className={`px-8 py-4 rounded-full border ${theme === 'light' ? 'border-gray-200 hover:bg-gray-100 text-black' : 'border-white/20 hover:bg-white/10 text-white'} font-bold tracking-wide transition-all flex items-center gap-2 group`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[280px]">
             {BENTO_ITEMS.map((item, i) => (
                <motion.div
                  ...
                  className={`group relative rounded-[2.5rem] overflow-hidden cursor-pointer ${item.className} ${theme === 'light' ? 'bg-gray-100' : 'bg-[#0f0f0f]'} border ${theme === 'light' ? 'border-gray-200' : 'border-white/10'}`}
                >
                   ...
                   <div className="absolute inset-0 p-8 flex flex-col justify-between">
                      <div className="flex justify-between items-start">
                         <div className={`p-3 rounded-full backdrop-blur-md ${theme === 'light' ? 'bg-white/90 text-black' : 'bg-black/40 text-white border border-white/20'}`}>{item.icon}</div>
                      ...
                      <div>
                         <span className="text-[#D4AF37] font-bold tracking-widest text-[10px] uppercase mb-2 block opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 delay-75">{item.subtitle}</span>
                         <h3 className="text-white text-2xl md:text-3xl font-black uppercase leading-tight tracking-tight drop-shadow-lg">{item.title}</h3>
                      </div>
                   </div>
                </motion.div>
             ))}
...
const ReviewSection = ({ theme }: { theme: 'dark' | 'light' }) => {
  return (
    <section className={`py-32 relative overflow-hidden ${theme === 'light' ? 'bg-gray-50' : 'bg-[#080808]'} border-t ${theme === 'light' ? 'border-gray-200' : 'border-white/10'}`}>
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-bl from-[#D4AF37]/10 to-transparent blur-[120px] rounded-full pointer-events-none" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-20">
          <div>
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className={`text-5xl md:text-6xl font-black uppercase tracking-tight leading-none mb-6 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
              Loved By <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#FCEEAC]">Collectors</span>
            </motion.h2>
            <p className={`text-lg max-w-md ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
          ...
          <motion.div 
            ...
            className={`p-8 rounded-[2rem] border backdrop-blur-xl ${theme === 'light' ? 'bg-white/80 border-gray-200 shadow-xl' : 'bg-white/5 border-white/10 shadow-2xl'}`}
          >
            <div className="flex items-center gap-8 mb-8">
              <div className="flex flex-col">
                 <span className={`text-6xl font-black ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>4.94</span>
                 <div className="flex items-center gap-1 mt-2">
                   {[1,2,3,4,5].map(i => <Star key={i} size={16} fill="#D4AF37" className="text-[#D4AF37]" />)}
                 </div>
                 <span className={`text-sm mt-1 ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>Based on 1,208 reviews</span>
              </div>
              <div className="h-16 w-px bg-current opacity-10" />
            ...
            <button className={`w-full py-4 rounded-xl font-bold tracking-wide transition-all transform hover:scale-[1.02] active:scale-[0.98] ${theme === 'light' ? 'bg-[#D4AF37] text-white shadow-lg shadow-[#D4AF37]/30' : 'bg-[#D4AF37] text-black shadow-[0_0_20px_#D4AF37]'}`}>
               Write a Review
            </button>
          </motion.div>
        </div>
        <div className="relative mb-20">
           <div className={`absolute left-0 top-0 bottom-0 w-32 z-10 bg-gradient-to-r ${theme === 'light' ? 'from-gray-50' : 'from-[#080808]'} to-transparent`} />
           <div className={`absolute right-0 top-0 bottom-0 w-32 z-10 bg-gradient-to-l ${theme === 'light' ? 'from-gray-50' : 'from-[#080808]'} to-transparent`} />
           <div className="flex gap-6 overflow-hidden mask-linear-fade pb-8">
              <motion.div 
                animate={{ x: "-50%" }} 
              ...
              >
                 {[...REVIEWS, ...REVIEWS, ...REVIEWS].map((review, i) => (
                    <div key={`${review.id}-${i}`} className={`w-80 p-6 rounded-2xl flex-shrink-0 border ${theme === 'light' ? 'bg-white border-gray-100 shadow-sm' : 'bg-[#111] border-white/5'}`}>
                       <div className="flex items-center gap-3 mb-4">
                          <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden">
                             <User className="w-full h-full p-2 text-gray-500" /> 
                          </div>
                          <div>
                             <h4 className={`font-bold text-sm ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>{review.name}</h4>
                             <div className="flex items-center gap-1">
                               {[...Array(review.rating)].map((_, r) => <Star key={r} size={10} fill="#D4AF37" className="text-[#D4AF37]" />)}
                             </div>
                          </div>
                          <span className={`ml-auto text-[10px] ${theme === 'light' ? 'text-gray-400' : 'text-gray-600'}`}>{review.date}</span>
                       </div>
                       <p className={`text-sm leading-relaxed ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>"{review.text}"</p>
                    </div>
                 ))}
              </motion.div>
           </div>
        ...
        <div>
           <div className="flex items-center justify-between mb-8">
              <h3 className={`text-2xl font-bold uppercase tracking-tight ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>Customer Gallery</h3>
              <a href="#" className={`text-sm font-bold border-b hover:text-[#D4AF37] hover:border-[#D4AF37] transition-colors ${theme === 'light' ? 'text-gray-900 border-gray-300' : 'text-white border-white/30'}`}>View All Photos</a>
           </div>
           <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {CUSTOMER_PHOTOS.map((photo, i) => (
                 <motion.div 
                   key={i} 
                   whileHover={{ y: -5 }}
                   className={`aspect-square rounded-2xl overflow-hidden cursor-pointer relative group ${theme === 'light' ? 'bg-gray-200' : 'bg-[#111]'}`}
                 >
...
const Footer = ({ theme }: { theme: 'dark' | 'light' }) => {
  return (
    ...
    <footer className={`relative pt-32 pb-12 ${theme === 'light' ? 'bg-white text-gray-900' : 'bg-[#050505] text-white'}`}>
       
       {/* Background Grid Pattern (Clipped inside absolute div) */}
       <div className="absolute inset-0 overflow-hidden pointer-events-none">
         <div className={`absolute inset-0 opacity-[0.03] ${theme === 'light' ? 'bg-[linear-gradient(to_right,#000_1px,transparent_1px),linear-gradient(to_bottom,#000_1px,transparent_1px)]' : 'bg-[linear-gradient(to_right,#fff_1px,transparent_1px),linear-gradient(to_bottom,#fff_1px,transparent_1px)]'} bg-[size:40px_40px]`} />
       </div>
       ...
       <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[280px] sm:w-[400px] md:w-[600px] pointer-events-none z-30 block">
          ...
          >
             ...
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[50%] bg-[#D4AF37]/20 blur-[60px] rounded-full -z-10" />
          </motion.div>
       </div>
       ...
       <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
             ...
             <div className="col-span-1 md:col-span-1">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <div className="w-12 h-12 relative rounded-full overflow-hidden border border-[#D4AF37]/50 shadow-[0_0_15px_rgba(212,175,55,0.3)]">
                      <img 
                        src="/wow-logo.png" 
                   ...
                   <span className="text-2xl font-black tracking-tighter">WOW <span className="text-[#D4AF37]">LIFESTYLE</span></span>
                </div>
                <p className={`text-sm leading-relaxed mb-6 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                ...
                <div className="flex gap-4">
                   {[Twitter, Instagram, Facebook, Youtube].map((Icon, i) => (
                      <a key={i} href="#" className={`p-3 rounded-full transition-all ${theme === 'light' ? 'bg-gray-100 hover:bg-[#D4AF37] hover:text-white' : 'bg-white/5 hover:bg-[#D4AF37] hover:text-black'}`}>
                         <Icon size={18} />
                      </a>
                   ))}
                </div>
             ...
             {/* Links Columns */}
             <div>
                <h4 className="font-bold mb-6 text-[#D4AF37] uppercase tracking-wider text-xs">Company</h4>
                <ul className={`space-y-4 text-sm ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
             ...
             <div>
                <h4 className="font-bold mb-6 text-[#D4AF37] uppercase tracking-wider text-xs">Support</h4>
                <ul className={`space-y-4 text-sm ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
             ...
             {/* Newsletter Column */}
             <div>
                <h4 className="font-bold mb-6 text-[#D4AF37] uppercase tracking-wider text-xs">Stay in the Lead</h4>
                <p className={`text-sm mb-4 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                   Subscribe to our pit stop newsletter for the latest drops.
                </p>
                <div className={`flex items-center p-1 rounded-xl border ${theme === 'light' ? 'bg-gray-50 border-gray-200' : 'bg-white/5 border-white/10'}`}>
                   <div className="pl-3 text-gray-400"><Mail size={18} /></div>
                   <input 
                     type="email" 
                     placeholder="Enter your email" 
                     className="w-full bg-transparent border-none focus:ring-0 text-sm p-3 outline-none"
                   />
                   <button className="p-3 rounded-lg bg-[#D4AF37] text-black font-bold hover:bg-[#FCEEAC] transition-colors">
                      <Send size={18} />
                   </button>
                </div>
             </div>
          ...
          <div className={`pt-8 border-t flex flex-col md:flex-row justify-between items-center gap-4 ${theme === 'light' ? 'border-gray-200 text-gray-500' : 'border-white/10 text-gray-500'}`}>
             <p className="text-xs">© 2024 WOW Lifestyle. All rights reserved.</p>
...
const StudioShowcase = ({ videos, theme }: { videos: typeof VINTAGE_VIDEOS, theme: 'dark' | 'light' }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto perspective-[1200px] py-10 group">
      <div className={`relative w-full aspect-[16/9] md:aspect-[2.35/1] rounded-[2rem] shadow-2xl overflow-hidden ${theme === 'light' ? 'bg-gray-100 shadow-gray-300' : 'bg-[#0a0a0a] shadow-black'} border border-white/5`}>
        <div className="absolute -inset-4 blur-[80px] opacity-20 transition-colors duration-1000 -z-10" style={{ backgroundColor: activeVideo.color }} />
        ...
        <div className="absolute top-8 left-8 z-20">
           <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10">
              ...
              <span className="text-[10px] font-medium text-white uppercase tracking-widest">Live Exhibit</span>
           </div>
        </div>
        <div className="absolute bottom-0 left-0 p-8 z-20 flex flex-col gap-1 transition-transform duration-500 group-hover:translate-x-2">
            <h3 className="text-white text-3xl font-light tracking-wide flex items-center gap-4">
              <span className="font-bold">No. {activeVideo.id.toString().padStart(2, '0')}</span>
              <span className="text-white/30 text-xl font-thin">|</span>
              <span className="text-white/80 text-xl font-light tracking-widest uppercase">Vintage Collection</span>
            </h3>
            <p className="text-white/40 text-xs tracking-widest uppercase mt-1">Auto-cycling • {activeVideo.color} Series</p>
        </div>
        <div className="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-30 pointer-events-none">
            <button onClick={handlePrev} className="pointer-events-auto p-4 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white hover:bg-white hover:text-black transition-all transform hover:scale-110"><ChevronLeft size={24} /></button>
            <button onClick={handleNext} className="pointer-events-auto p-4 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white hover:bg-white hover:text-black transition-all transform hover:scale-110"><ChevronRight size={24} /></button>
        </div>
        <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/10 z-30">
           <motion.div className="h-full bg-[#D4AF37] shadow-[0_0_10px_#D4AF37]" style={{ width: `${timer}%` }} />
        </div>
      </div>
    </div>
  );
...
export default function LandingPage() {
  ...
  return (
    <div className={`w-full ${getBackgroundColor()} ${getTextColor()} transition-colors duration-300`}>
      {/* HERO SECTION */}
      <section className={`relative min-h-screen overflow-hidden flex flex-col justify-center pt-24 lg:pt-32 pb-20`}>
        <div className="absolute right-[-10%] top-[20%] w-[600px] h-[600px] bg-gradient-to-br from-[#D4AF37]/20 to-transparent blur-[150px] rounded-full pointer-events-none -z-10" />
        <div className={`absolute inset-0 ${getGridColor()} bg-[size:24px_24px] -z-20`}></div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="flex flex-col justify-center text-center lg:text-left z-10 order-2 lg:order-1">
                ...
                <motion.h1 custom={0.2} initial="hidden" animate="visible" variants={textVariants} className={`text-5xl sm:text-6xl lg:text-7xl font-black ${getTextColor()} leading-tight mb-6`}>Race Ready. <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#FCEEAC] to-[#D4AF37] animate-gradient">Miniature Speed.</span></motion.h1>
                <motion.p custom={0.4} initial="hidden" animate="visible" variants={textVariants} className={`${getSecondaryTextColor()} text-lg max-w-lg mx-auto lg:mx-0 mb-10 leading-relaxed`}>Experience the thrill of the track with our ultra-realistic, precision-engineered diecast Formula 1 collection.</motion.p>
                <motion.div custom={0.6} initial="hidden" animate="visible" variants={textVariants} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                  <button className="px-8 py-4 bg-gradient-to-r from-[#D4AF37] to-[#FCEEAC] text-black font-bold text-lg rounded-xl hover:shadow-xl hover:shadow-[#D4AF37]/25 transition-all duration-300 flex items-center justify-center gap-3 group">Shop Collection <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></button>
        ...
        <div className={`relative w-full border-t ${getBorderColor()} ${theme === 'light' ? 'bg-white/60' : 'bg-black/60'} backdrop-blur-lg mt-12 py-6`}>
           <div className={`absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r ${theme === 'light' ? 'from-white' : 'from-black'} to-transparent z-10 pointer-events-none`} />
           <div className={`absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l ${theme === 'light' ? 'from-white' : 'from-black'} to-transparent z-10 pointer-events-none`} />
           <div className="flex overflow-hidden">
              <motion.div className="flex items-center gap-16 md:gap-24 px-12" animate={{ x: "-50%" }} transition={{ duration: 40, ease: "linear", repeat: Infinity }}>
                 {[...BRAND_LOGOS, ...BRAND_LOGOS].map((brand, i) => ( <div key={`${brand.name}-${i}`} className="group relative flex-shrink-0 cursor-pointer"><img src={brand.src} alt={brand.name} className={`h-10 md:h-12 w-auto object-contain transition-all duration-500 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-125 ${theme === 'light' ? 'grayscale opacity-50' : 'grayscale opacity-40'}`} /></div> ))}
              </motion.div>
           </div>
        </div>
      </section>
      ...
      {/* TRENDING NOW */}
      <section className={`relative py-24 ${getSectionBackground()} border-t ${getBorderColor()}`}>
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-[500px] bg-gradient-to-b from-[#D4AF37]/10 to-transparent blur-[120px] rounded-full pointer-events-none" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="text-center md:text-left">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-[#D4AF37]/10 to-transparent border border-[#D4AF37]/20 mb-4">
                <Zap size={16} className="text-[#D4AF37]" /><span className="text-sm font-bold text-[#D4AF37] tracking-wider">TRENDING NOW</span>
              </div>
              <h2 className={`text-4xl md:text-6xl font-black ${getTextColor()} mb-4`}>Hot <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#FCEEAC]">Drops</span></h2>
              <p className={`${getSecondaryTextColor()} text-lg max-w-xl`}>See what's hot in the collector's world this week.</p>
            </div>
            <button className={`group flex items-center gap-3 ${getTextColor()} font-bold hover:text-[#D4AF37] transition-colors px-6 py-3 rounded-xl border ${getBorderColor()} hover:border-[#D4AF37]/50`}>View All <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></button>
          </div>
      ...
      <section className={`relative py-32 ${theme === 'light' ? 'bg-gray-50' : 'bg-black'} overflow-hidden border-t ${getBorderColor()}`}>
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none"><div className={`absolute top-0 left-1/2 -translate-x-1/2 w-full h-[500px] ${theme === 'light' ? 'bg-gradient-to-b from-white to-transparent' : 'bg-gradient-to-b from-[#111] to-transparent'}`} /></div>
        <div className="container mx-auto px-4 relative z-20">
          <div className="text-center mb-16">
            <h2 className={`text-5xl md:text-7xl font-black tracking-tighter leading-none mb-4 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>STUDIO <span className="text-[#D4AF37]">SHOWCASE</span></h2>
            <p className={`max-w-2xl mx-auto text-lg ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>Watch our premium vintage models in action. <br className="hidden md:block"/>Fast-paced automotive exhibition.</p>
          </div>
